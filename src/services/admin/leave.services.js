...
function getApprovedLeavesList(search) {
    const requestOptions = {
        method: "POST",
        headers: { ...authHeader(), "Content-Type": "application/json" },
        body: JSON.stringify(search)
    ...
    return fetch(APIURL+"leaves/getApprovedLeaves", requestOptions)
        .then(handleResponse)
        .then(result => {
            return result;
        });
...
function checkCasualLeaves(user) {
    ...
    return fetch(APIURL+"leaves/totalCasualLeave", requestOptions)
        .then(handleResponse)
        .then(result => {
            console.log("data levescount calual=====>>>>>",result);
