...
class Index extends Component {
  ...
  getReportingList() {
    if (this.state.user) {
      this.props.dispatch(
        userActions.getReportingList({ reportTo: this.state.user.data._id })
      );
    }
  }
  ...
  leaveType() {
    leaveService.probationEndDate({userId:this.state.user.data._id})
    .then((res)=>{
      this.setState({probation_end_date:res.data[0].users.probation_end_date});
      if (this.state.user) {
        let leaveType = [];
       
        const date = new Date();
        
        let ProbationDate = dateFormat(
          res.data[0].users.probation_end_date,
          "yyyy-mm-dd"
        );
        // var nextMonth = new Date(joinDate);
        // ProbationDate = nextMonth.setMonth(nextMonth.getMonth() + 2);

        if (new Date(ProbationDate) < date||res.data[0].users.probation_end_date ==null) {
                    ...
                    LeaveList.map((item, index) => (
                      <tr key={index}>
                        <td>
                          ...
                          <small>
                            Applied on:{" "}
                            {dateFormat(item.leaves.createdAt, "ddd, dd mmm")}
                          </small>
                          <div>
                            <small>
                              <strong>
                        ...
                        <td className="text-right">
                          {this.state.filterReportUser &&
                            ...
                            this.state.user.data._id ==
                              this.state.filterReportUser[0].value &&
                            item.leaves.status === "pending" && (
