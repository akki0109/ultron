...
//   getProject,
//   graphDataApiCall,
// } from "../../../../services/admin/projectAnalytics.services";
import {
  getProject,
  graphDataApiCall,
} from "../../../../services/projectAnalytics.services";
...
import {
  allMonth,
  monthNames,
  optionsProjectAnalytics,
  ChartColor,
  ProjectAnalyticsPieChart,
...
const Index = () => {
  ...
  useEffect(() => {
    getProject(user).then((item) => {
      ...
      if (reportingList.length) {
        setSelectProject({
          value: reportingList[0].value,
          label: reportingList[0].label,
        });
      }
    });
  }, []);
    ...
    graphDataApiCall({
      ...
      if (response?.data) {
        ...
        response.data?.map((item) => {
          ...
          if (pieProjectLabel.includes(item.userData[0].first_name) != true)
            pieProjectLabel.push(item.userData[0].first_name);
          if (
            projects.filter(
              (itemProject) => itemProject.id == item?.userData[0]?._id
            ).length == 0
          ) {
            projects.push({
              name:
      ...
      project.map((itemProject, index) => {
        ...
        filterData = graphData.filter(
          (iteamGraph) =>
            ...
            iteamGraph.projecttasks.userId == itemProject.id
        );
  ...
  const totalHour = useMemo(() => {
    ...
    for (const arr of stackDataArragement) {
      ...
      totalHourPerEmployee.push(count);
      count = 0;
    }
    //for time calculate
    hour = hour.toFixed(2);
  ...
  return (
    <div>
      ...
      <main className="offset">
        <div className="container-fluid hard-pad">
          <div className="row align-items-center pt-4 border-mobile">
            ...
            <div className="col-md-3 offset-md-5 col-6 zindex2">
              <Select
          ...
          <div className="mt-5">
            <DetailSheet
              ...
              setToggle={setToggle}
              month={Month}
              project={selectProject?.label}
            />
            <Bar
              options={optionsProjectAnalytics}
              data={stackedData}
              redraw={true}
            />
          </div>
      ...
      <div className="right-aside">
        ...
        <div className="mt-4">
          <Pie
            options={ProjectAnalyticsPieChart}
            data={pieData}
            redraw={true}
          />
        </div>
