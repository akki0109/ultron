...
class Index extends Component {
  constructor(props) {
    ...
    this.getFloatingLeaveCount = this.getFloatingLeaveCount.bind(this);
  }
  ...
  closeModal = () => {
    ...
    this.props.dispatch(leaveActions.emptyError());
  };

  sendLeaveRequest = () => {
    if (
      this.state.LeaveDaysCount > 0 ||
      this.state.startDate == "" ||
      this.state.endDate == ""
    ) {
      this.setState({ error: "" });
      this.props.dispatch(
        leaveActions.sendLeaveRequest({
          userId: this.state.userId,
          from_date:
            this.state.startDate !== ""
              ? dateFormat(this.state.startDate, "yyyy-mm-dd")
              : "",
          to_date:
            this.state.endDate !== ""
              ? dateFormat(this.state.endDate, "yyyy-mm-dd")
              : "",
  ...
  openEditModel(data) {
    ...
    var userNameForEdit = "";
    var leaveUserList1 = leaveUserList.filter(
  ...
  updateSendLeaveRequest(userId) {
    if (
      ...
      this.props.dispatch(
        leaveActions.updateRequestedLeave({
          userId: userId,
          id: this.state.leave_id,
          from_date:
            this.state.startDate !== ""
              ? dateFormat(this.state.startDate, "yyyy-mm-dd")
              : "",
          ...
          note: this.state.note,
          leave_type:
            this.state.leaveType.value === "null"
              ? ""
  ...
  getLeavesList() {
    ...
    this.props.dispatch(
      leaveActions.getLeavesList({
        page: this.state.activePage,
        userId: reportUser,
        search: "",
        reportTo: "",
        leaveStatusFilter: this.state.filterStatus[0].value,
  ...
  getCasualLeaveData() {
    if (this.state.filterReportUser && this.state.filterReportUser.length > 0) {
      this.props.dispatch(
        leaveActions.checkCasualLeaves({
          _id: this.state.filterReportUser[0].value,
        })
      );
    }
  }
  ...
  getUnpaidLeaveCount() {
    if (this.state.filterReportUser && this.state.filterReportUser.length > 0) {
      this.props.dispatch(
        leaveActions.checkUnpaidLeave({
          userId: this.state.filterReportUser[0].value,
        })
      );
    }
  }
  getFloatingLeaveCount = async () => {
    let res = await leaveService.checkFloatingLeaves({
      userId: this.state.filterReportUser[0].value,
    });
  ...
  getReportingList() {
    if (this.state.user) {
      this.props.dispatch(
        adminUserActions.getUserListForReporting({ search: "" })
      );
    }
  }
  ...
  getDiff = () => {
    ...
    if (diffDays == 0) {
      if (currentDate.getDay() == 6 || currentDate.getDay() == 0) {
        ...
        count =
          this.state.activeFrom == "first_half" ||
          this.state.activeFrom == "second_half"
            ? 0.5
            : 1;
      }
    }
    if (diffDays > 0) {
      ...
      if (
        this.state.activeto == "first_half" &&
        endDate.getDay() != 6 &&
        endDate.getDay() != 0
      ) {
        count = count - 0.5;
      ...
      this.state.holidayList.map((item) => {
        if (
          item == dateFormat(this.state.startDate, "yyyy-mm-dd") &&
          this.state.activeFrom == "second_half" &&
          currentDate.getDay() != 6 &&
          currentDate.getDay() != 0
        ) {
        ...
        if (
          item == dateFormat(this.state.endDate, "yyyy-mm-dd") &&
          this.state.activeto == "first_half" &&
          endDate.getDay() != 6 &&
          endDate.getDay() != 0
        ) {
  ...
  leaveType() {
    if (this.state.user) {
      ...
      var JoiningDate = [];
      const date = new Date();
      let joinDate = dateFormat(
        this.state.user.data.date_of_joining,
        "yyyy-mm-dd"
      );
      ...
      if (JoiningDate < date) {
        ...
        leaveType.push({ value: "floating leave", label: "Floating Leave" });
      }
  ...
  filterHolidayList() {
    if (this.state.startDate != "" && this.state.endDate != "") {
      ...
      var separator = "-";
      sdate = `${sdate.getFullYear()}${separator}${
        sdate.getMonth() + 1 < 10
          ? `0${sdate.getMonth() + 1}`
          : `${sdate.getMonth() + 1}`
      }${separator}${
      ...
      edate = `${edate.getFullYear()}${separator}${
        edate.getMonth() + 1 < 10
          ? `0${edate.getMonth() + 1}`
          : `${edate.getMonth() + 1}`
      }${separator}${
  ...
  setHolidayFilter(data) {
    ...
    data.map((item) => {
      ...
      while (getCurrentDate <= getEndDate) {
        holidaResult.push(
          `${getCurrentDate.getFullYear()}${separator}${
            getCurrentDate.getMonth() + 1 < 10
              ? `0${getCurrentDate.getMonth() + 1}`
              : `${getCurrentDate.getMonth() + 1}`
          }${separator}${
            getCurrentDate.getDate() < 10
              ? `0${getCurrentDate.getDate()}`
              : `${getCurrentDate.getDate()}`
          }`
        );
        getCurrentDate = new Date(
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (
      nextProps.LeaveActivePage != undefined &&
      nextProps.LeaveTotalItemsCount != undefined &&
      nextProps.LeaveLimit != undefined
    ) {
      this.handlePageStates(
        nextProps.LeaveActivePage,
        nextProps.LeaveTotalItemsCount,
        10
      );
    }
    ...
    if (nextProps.EditList != undefined && this.state.startDate == "") {
      var leaveType = { value: "unpaid leave", label: "Unpaid Leave" };
      if (nextProps.EditList.leave_type === "casual leave") {
        leaveType = { value: "casual leave", label: "Casual Leave" };
      } else if (nextProps.EditList.leave_type === "sick leave") {
        leaveType = { value: "sick leave", label: "Sick Leave" };
      } else if (nextProps.EditList.leave_type === "floating leave") {
        leaveType = { value: "floating leave", label: "Floating Leave" };
      }
      this.setState(
        {
          ...
          activeFrom:
            nextProps.EditList.from_date_half == ""
              ? "full_day"
              : nextProps.EditList.from_date_half,
          activeto:
            nextProps.EditList.to_date_half == ""
              ? "full_day"
              : nextProps.EditList.to_date_half,
        ...
        <main className="offset">
          <div className="container-fluid hard-pad">
            <div className="row align-items-center pt-4 border-mobile">
              ...
              <div className="col-lg-8 text-right">
                <a
                  href="javascript:;"
                  data-toggle="modal"
                  data-target="#applyLeave"
                  onClick={this.openModel.bind(this, this.state.user.data)}
                  className="link"
                >
                  Apply Leave
                </a>
                ...
                >
                  ...
                  <li>
                    <Select
                      ...
                      value={this.state.filterReportUser}
            ...
            <div id="table-scroll" className="table-scroll mt-4 pagination_mrg">
              <table
                id="main-table"
                className="main-table full-first-td pagination_show"
              >
                <thead>
                  <tr>
                    ...
                    LeaveList.map((item, index) => (
                      <tr key={index}>
                        <td>
                          ...
                          <small>
                            Applied on:{" "}
                            {dateFormat(item.leaves.createdAt, "ddd, dd mmm yyyy")}
                          </small>
                          <div>
                            <small>
                              <strong>
                                ...
                                -{" "}
                                {dateFormat(item.leaves.to_date, "ddd, dd mmm yyyy")}
                              </strong>
                            </small>
                          </div>
                          <div>
                            <small>
                              {dateFormat(
                                ...
                                dateFormat(item.leaves.to_date, "yyyy-mm-dd") &&
                        ...
                        <td>
                          ...
                          {item.leaveRequestBy[0].reportToData.length > 0 && (
                            <div className="text-muted small">
                              (RM -{" "}
                              {
                                item.leaveRequestBy[0].reportToData[0]
                                  .first_name
                              }{" "}
                              {item.leaveRequestBy[0].reportToData[0].last_name}
                              )
                            </div>
                          )}
                        </td>
                        <td data-th="" className="">
                          {this.Capitalize(item.leaves.leave_type)}
                          <div>
                            <small>{item.leaves.note}</small>
                          </div>{" "}
                        </td>
                        {item.leaves.status === "pending" ? (
                          <td data-th="" className="text-center">
                            <span className="capsule red-dim ">
                              {item.leaves.status}
                            </span>
                          </td>
                        ) : item.leaves.status === "reject" ? (
                          <td data-th="" className="text-center">
                            <span className="capsule Blackberry-dim ">
                              rejected
                            </span>
                          </td>
                        ) : (
                              ...
                              <div className="action-area dropdown">
                                ...
                                >
                                  <a
                                    ...
                                    href=""
                                    data-toggle="modal"
                                    onClick={this.openEditModel.bind(
                                      this,
                                      item.leaves
                                    )}
        ...
        <div className="right-aside">
          ...
          <div className="stats">
            <div className="stats-big">
              {CLeaveData && CLeaveData.consumedLeave}/
              {CLeaveData && CLeaveData.totalLeave}
            </div>
            <div className="stats-small">Casual Leaves</div>
          </div>
          <div className="stats">
            <div className="stats-big">
              {this.state.FLeaveData && this.state.FLeaveData.consumedLeave}/
              {this.state.FLeaveData && this.state.FLeaveData.totalLeave}
            </div>
            <div className="stats-small">Floating Leaves</div>
          </div>
          <div className="stats">
            <div className="stats-big">
              {SLeaveData && SLeaveData.consumedLeave}/
              {SLeaveData && SLeaveData.totalLeave}
            </div>
            <div className="stats-small">Sick Leaves</div>
          </div>
          <div className="stats">
            <div className="stats-big">
              {UnpaidLeaveData && UnpaidLeaveData.consumedLeave}/∞
            </div>
          ...
          <div className="modal-content">
              ...
              <div className="my-2">
                <div className="form-row">
                  <div className="col">
                    <div className="form-group">
                      ...
                      <Select
                        styles={colourStyles}
                        onChange={this.employeeChangeHandle}
                        value={this.state.formUser}
                        options={this.state.leaveUserList}
                      />
                    </div>
                  </div>
                </div>
                <div className="form-row">
                  <div className="col">
                    <div className="form-group">
                      ...
                      {error ? (
                        <span className="text-danger"> {error.from_date}</span>
                      ) : (
                        ""
                      )}
                    </div>
                  ...
                  <div className="col">
                    <div className="form-group">
                      ...
                      {error ? (
                        <span className="text-danger"> {error.to_date}</span>
                      ) : (
                        ""
                      )}
                    </div>
                ...
                {this.state.openModel && this.state.diffDays == 0 ? (
                  <div className="leave-rows my-3">
                    <div className="form-row align-items-center border-top border-bottom py-2">
                      <div className="col-4">
                        {dateFormat(this.state.startDate, "dd mmm yyyy")}
                      </div>
                      <div className="col-8">
                          ...
                          <ul className="pagination my-0">
                            {HalfDays.map((shift, idx) => (
                              <li
                                ...
                                className={
                                  "page-item" +
                                  (this.state.activeFrom == shift.value
                                    ? " active"
                                    : "")
                                }
                              >
                                <a
                                  onClick={() =>
                                    this.setShift(shift.value, "from_half_day")
                                  }
                ...
                {this.state.openModel && this.state.diffDays > 0 ? (
                  <div className="leave-rows my-3">
                    ...
                    <div className="form-row align-items-center border-top border-bottom py-2">
                      <div className="col-4">
                        {dateFormat(this.state.endDate, "dd mmm yyyy")}
                      </div>
                      <div className="col-8">
                        <nav aria-label="Page navigation">
                          <ul className="pagination my-0">
                            {toHalfDays.map((shift, idx) => (
                              <li
                                ...
                                className={
                                  "page-item" +
                                  (this.state.activeto == shift.value
                                    ? " active"
                                    : "")
                                }
                              >
                                <a
                                  onClick={() =>
                                    this.setShift(shift.value, "to_half_day")
                                  }
                                  className="page-link  active"
                                  href="javascript:;"
                                >
                ...
                <div className="form-group">
                  ...
                  <div className="form-group select_height">
                    ...
                    />
                    {error ? (
                      <span className="text-danger"> {error.leave_type}</span>
                    ) : (
                      ""
                    )}
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    ></textarea>
                    {error ? (
                      <span className="text-danger"> {error.note}</span>
                    ) : (
                      ""
                    )}
                  </div>
                ...
                <div className="col-12 p-0">
                  {ScsMsg && (
                    <span className="text-success">{ScsMsg.message}</span>
                  )}
                  {error ? (
                    <span className="text-danger"> {error.common}</span>
                  ) : (
                    ""
                  )}
                  {this.state.startDate != "" &&
                  ...
                  this.state.LeaveDaysCount <= 0 ? (
                    <span className="text-danger">
                      Leave count should be greater than 0
                    </span>
                  ) : (
                    ""
                  )}
            ...
            <ModalFooter className="modal-footer">
              ...
              {loading ? (
                ...
                <Button color="primary" onClick={this.sendLeaveRequest}>
                  Request Leave
                </Button>
              )}
            </ModalFooter>
          ...
          <div className="modal-content">
            ...
            <ModalBody className="modal-body">
              <div className="my-2">
                ...
                <div className="form-group">
                  <label>
                    Leave Type<strong className="text-danger"> *</strong>
                  </label>
                  <Select
                    styles={colourStyles}
                    placeholder="Type"
              ...
              <div className="col-12 p-0">
                {LeaveScsMsg ? (
                  <span className="text-success">{LeaveScsMsg.message}</span>
                ) : (
                  ""
                )}
                {error ? (
            ...
            <ModalFooter className="modal-footer">
              ...
              {loading ? (
                ...
                <Button
                  color="primary"
                  onClick={this.updateSendLeaveRequest.bind(
                    this,
                    this.state.userId
                  )}
                >
          ...
          <div className="modal-content">
            ...
            <ModalFooter>
              ...
              {loading ? (
                ...
                <Button
                  color="primary"
                  onClick={this.deleteLeaveRequest.bind(
                    this,
                    this.state.deleteLeaveId
                  )}
                >
                  Delete
                </Button>
