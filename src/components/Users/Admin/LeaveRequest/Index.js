...
// const Button123 = React.lazy(() => import("MicroFrontend/Button"));
...
class Index extends Component {
    ...
    // getCasualLeaveData() {
    //     console.log("==========cid================================",this.state.user.data._id);
    //     this.props.dispatch(adminLeaveActions.checkCasualLeaves({ _id: this.state.user.data._id }));
    // }

    // getSickLeaveData() {
    //     console.log("==========cid================================",this.state.user.data._id);

    //     this.props.dispatch(adminLeaveActions.checkSickLeaves({ _id: this.state.user.data._id }));
    // }
            ...
            <div>
                    ...
                    <div className="container-fluid hard-pad">
                        ...
                        <div id="table-scroll" className="table-scroll mt-4 pagination_mrg">
                            <table id="main-table" className="main-table full-first-td">
                                <thead>
                                    <tr>
                                        <th scope="col" className="col-3">Leave Date</th>
                                        <th scope="col" className="col-2">Employee Name</th>
                                        <th scope="col" className="col-4">Type/Reason</th>
                                        <th scope="col" className="text-center col-1">Status</th>
                                ...
                                <tbody>
                                    ...
                                    {this.state.LeavesRequestsList && this.state.LeavesRequestsList.map((item, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                {item.leaveRequestBy[0].first_name} {item.leaveRequestBy[0].last_name}
                                                {item.leaveRequestBy[0].reportToData.length >0 &&
                                                    <div className="text-muted small">(RM - {item.leaveRequestBy[0].reportToData[0].first_name} {item.leaveRequestBy[0].reportToData[0].last_name})</div>
                                                }
                                            </td>
                                            <td data-th="" className="text-left">
                                                {this.Capitalize(item.leaves.leave_type)}
                                                <div><small>{item.leaves.note}</small></div> 
                                            </td>
                                            ...
                                            <td data-th="Order Value" className="text-center">
                                                {item.leaves.status == "pending" ?
                                                    <div className="action-area text-center d-flex justify-content-center">
                                                        <a onClick={this.openModel.bind(this, item.leaves)} className="btn btn-success btn-small m-1">Approve</a>
                                                        <a href="javascript:;" data-toggle="modal" data-target="#RejectLeave" className="btn btn-danger btn-small m-1" onClick={this.openRejectModel.bind(this, item.leaves)}>Reject</a>
                                                    </div>
                                                    : ""}
                        ...
                        <ModalBody className="border-0">
                            ...
                            <div id="table-scroll" className="table-scroll h-auto popup_table my-3 modal_table">
                                <table id="main-table" className="main-table full-first-td">
                                    <thead>
                                        <tr>
                                            <th scope="col" className="text-left col-6">Leave Date</th>
                                            <th scope="col" className="text-right col-6">Type/Reason</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                ...
                <Modal size="ml" isOpen={this.state.Modal} toggle={() => this.closeModal()}>
                    <div className="modal-content">
                        ...
                        <ModalFooter>
                            <button className="btn btn-secondary mr-3" onClick={() => this.closeModal()}> Cancel</button>
                            {
                                loading ?
                ...
                {/* <Suspenseyyy>
                <Abhisek  Name={"Photography"}/>
                <Button123 buttonName={"Hey there"} />
                </Suspenseyyy> */}
                </div>
            </div>
        );
