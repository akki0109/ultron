...
import { adminLeaderBoardService } from "../../../../services//admin/leaderboard.services";
...
const LeaderBoard = () => {
  ...
  const getDepartmentList = () => {
    adminLeaderBoardService
      .getDepartmentList({ search: "" })
      .then((result) => {
        let newArrayObj = result.data.map((item) => {
          return { label: item.name, value: item._id };
        });
        newArrayObj.unshift({ value: "", label: "Select All" });
        setDepartmentList(newArrayObj);
      })
              ...
              >
                {noRecord == true &&
                  ...
                  leaderList?.map((item, index) => (
                    <div key={index} className="leader-ranking">
                      ...
                      {index > 2 && (
                        <div>
                          <h1 className="circular-pts circular-points mt-3" style={{marginLeft:"12px"}}>
                            {index + 1}
                          </h1>
                        </div>
                      )}
                ...
                {noRecord == true && leaderList?.length == 0 && (
                  <div className="leader-ranking">
                    <div className="text-white h5 text-center w-100">
                      No winners yet!
                    </div>
                  </div>
                )}
                  ...
                  topCaption.map((item, index) => (
                    <>
                      <div className="topCaption-leader">
                        ...
                        <div>
                          {index == 0 && <img src={firstLeader} alt="Image" />}
                          {index == 1 && (
                            <img
                              src={secondLeader}
                              width="100%"
                          ...
                          {index == 2 && <img src={thirdLeader} alt="Image" />}
                          {index > 2 && (
                            <h1 className="circular-points pb-0 mb-0 mt-3"  style={{ marginLeft: "35px" }}>
                              {index + 1}
                            </h1>
                          )}
                        </div>
