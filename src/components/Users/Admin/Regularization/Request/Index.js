...
import {
  getTimeTabel,
  updateTimeTabel,
} from "../../../../../services/admin/regularization.services";
...
const Index = (props) => {
  ...
  const getData = () => {
    getTimeTabel(props.approveInstance).then((Result) => {
  ...
  const saveChanges = () => {
    ...
    if (arraySortedOrNot(arr, n) != 0) {
      setError("");
      updateTimeTabel(updatedData).then((Result) => {
        ...
        <Modal.Header>
          <Modal.Title>
            <h5>
              ...
              <span className="small text-muted"> {dateFormat(props.approveDate, "fullDate")}</span>
            </h5>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
